...
class JamanagerApp {
    ...
    async createSong() {
        ...
        const chordSheetUrl = document.getElementById('chordSheetUrl').value.trim();
        ...
        if (chordSheetUrl && !this.isValidUrl(chordSheetUrl)) {
            this.showError('createSongError', 'Please enter a valid URL for the chord sheet');
            return;
        }
        ...
        try {
            ...
            // Only include chord_sheet_url if it's provided
            if (chordSheetUrl) {
                requestBody.chord_sheet_url = chordSheetUrl;
            }
            ...
            if (response.ok) {
                ...
                const successMessage = chordSheetUrl 
                    ? `Song "${title}" by ${artist} added successfully with chord sheet!`
                    : `Song "${title}" by ${artist} added successfully!`;
                this.showSuccess('createSongSuccess', successMessage);
                setTimeout(() => {
    ...
    selectChordSheet(url) {
        document.getElementById('chordSheetUrl').value = url;
        this.showChordSheetMessage('Chord sheet URL selected! You can now add the song.', 'success');
    }
    ...
    async verifyAccessCode() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.session_id) {
                    localStorage.setItem('jam_session_id', data.session_id);
                }
    ...
    async logoutJamManager() {
        try {
            ...
            if (response.ok) {
                ...
                localStorage.removeItem('jamManagerAccess');
                // Clear the jam manager session ID
                localStorage.removeItem('jam_session_id');
...
function selectChordSheet(url) {
    app.selectChordSheet(url);
}
